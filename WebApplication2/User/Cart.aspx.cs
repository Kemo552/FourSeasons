...
namespace WebApplication2.User
{
    public partial class Cart : System.Web.UI.Page
    {
        ...
        {
            WebApplication2.Connection.Utils utils = new Connection.Utils();
                ...
                for (int item = 0; item < rCartItem.Items.Count; item++)
                {
                    if (rCartItem.Items[item].ItemType == ListItemType.Item || rCartItem.Items[item].ItemType == ListItemType.AlternatingItem)
                    {
                        ...
                        if (isTrue)
                        {
                            ...
                            lblMsg.Visible = true;
                            lblMsg.Text = "Item updated successfully in your cart";
            ...
            if (e.CommandName == "checkout")
            {
                ...
                for (int item = 0; item < rCartItem.Items.Count; item++)
                {
                    if (rCartItem.Items[item].ItemType == ListItemType.Item || rCartItem.Items[item].ItemType == ListItemType.AlternatingItem)
                    {
                        ...
                        Label _productName = rCartItem.Items[item].FindControl("lblName") as Label;
                        int productId = Convert.ToInt32(_productId.Value);
                ...
                {
                    lblMsg.Visible = true;
                    lblMsg.Text = "Item <b>'" + productName + "'</b> is out of stock.";
        ...
        private sealed class CustomTemplate : ITemplate
        {
            ...
            public void InstantiateIn(Control container)
            {
                ...
                {
                    var footer = new LiteralControl("<tr><td class='user_option'><b>Your cart is empty.</b><a href='Menu.aspx' class='order_online'>Continue Shopping</a></td></tr></tbody></table>");
